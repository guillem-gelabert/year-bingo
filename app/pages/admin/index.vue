<template>
  <div class="p-4 py-8">
    <div class="max-w-6xl mx-auto">
      <!-- Page Title -->
      <div class="mb-8">
        <h1 class="text-4xl font-bold text-indigo-900 mb-2">Panell d'administració</h1>
        <p class="text-gray-600">Gestiona usuaris i visualitza bingos</p>
      </div>

      <!-- Generate New User Form -->
      <div class="bg-white rounded-lg shadow-xl p-6 mb-8">
        <h2 class="text-2xl font-bold text-indigo-900 mb-4">Crea un usuari nou</h2>
        <form @submit.prevent="handleGenerateUser" class="space-y-4">
          ...
          >
            {{ generating ? "Creant..." : "Crear usuari" }}
          </button>
          ...
          <div v-if="generateSuccess" class="mt-4 p-4 bg-green-50 rounded-lg border border-green-200">
            <p class="text-green-800 font-semibold mb-2">✅ Usuari creat!</p>
      ...
      <div class="bg-white rounded-lg shadow-xl p-6">
        <h2 class="text-2xl font-bold text-indigo-900 mb-4">Tots els usuaris</h2>
        <div v-if="loadingUsers" class="text-center py-12">
          <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mx-auto"></div>
          <p class="mt-4 text-gray-600">Carregant usuaris...</p>
        </div>
        <div v-else-if="users.length === 0" class="text-center py-12 text-gray-600">
          No s'han trobat usuaris. Crea el primer usuari aquí dalt!
        </div>
            ...
            <tbody>
              ...
              >
                ...
                <td class="py-3 px-4">
                  ...
                  <span v-else class="text-gray-500 text-sm">Usuari</span>
                </td>
              </tr>
            </tbody>
          </table>
...
const handleGenerateUser = async () => {
  try {
    ...
    generateError.value = error.data?.message || error.message || "No s'ha pogut crear l'usuari";
  } finally {
